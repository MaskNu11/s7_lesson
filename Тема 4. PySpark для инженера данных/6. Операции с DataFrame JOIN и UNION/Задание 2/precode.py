...
df_lib = spark.createDataFrame(data=library, schema=columns_library)
# напишите ваш код ниже

joined = df_lib.join(df, ['book_id'], how='semi')

# df_lib.select('book_id').distinct().show()
joined = df.join(df_lib, ['book_id'], how='anti')
row = joined.select('title').distinct()
print(row.count())